...
function Sidebar({ activeMenu: propActiveMenu, setActiveMenu: propSetActiveMenu }) {
  ...
  const menuItems = [
    ...
    },
    {
      name: 'Transactions',
      icon: (
        <svg className="w-6 h-6 animate-pulse-slow" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Icon */}
              <div className={`transition-transform duration-300 ${isActive ? 'text-[#1814F3] scale-110' : 'text-[#B1B1B1] group-hover:scale-105 group-hover:text-[#343C6A]'}`}>
                {item.icon}
              </div>

              {/* Text */}
              <span className={`text-[16px] font-semibold tracking-wide ${isActive ? 'font-bold' : 'font-medium'}`}>
                {item.name}
              </span>
            </div>
          );
